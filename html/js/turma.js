...
async function Insert() {
    ...
    const response = await fetch(`/ControllerTurma.php`, opt);
...
async function insert() {
    ...
    Alerta.innerHTML = response.msg;
    setTimeout(function () {
        window.location.href = `/listaturma.php?id=${response.id}`;
    }, 2000);
    //Exibir a mensagem de salvo com sucesso
    //Depois de exibir a mensagem de salvo com sucesso
    //Vamos direcionar para a pagina de listagem de turmas,
    // de forma automatica.
}
async function update() {
    ...
    Alerta.className = 'Alert alert-primary';
    Alerta.innerHTML = 'Salvando, por favor aguarde...'
    const request = await fetch('/ControllerTurma.php', opt);
